...
OBJECT_COMPLIANCE = {'bowl':0.2, 'cup':0.9, 'bottle':0.9}

CONFIDENCE_THRESHOLD = 0.5
NMS_THRESHOLD = 0.2
SEARCH_OBJECTS = ['cup', 'bottle', 'bowl', 'sports ball']
...
def moveToChargingPort(search_mode):
    setThymioSpeed(TESTING_SPEED)
    movable_object = False
    gyro_z = []
    flip = True

    print("getting to charging point...")
    prev = time.time()
    

    while not search_mode:
            ax,ay,az,wx,wy,wz = mpu6050_conv()
            # change this to be mean value
...
def testCompliance():
    c_mean, c_error, z_mean, z_error = calibrateThymio()
    time.sleep(2)
    setThymioSpeed(TESTING_SPEED)
